...
describe('gridSystem', () => {
  describe('tile generation', () => {
    it('isPassable returns a boolean for every valid coordinate', () => {
      ...
      for (let row = 0; row < ROWS; row++) {
        for (let col = 0; col < COLS; col++) {
          expect(typeof isPassable(worldEid, col, row)).toBe('boolean')
        }
      }
    })
    ...
    it('returns true for a passable tile', () => {
      ...
      expect(isPassable(worldEid, passableCol, passableRow)).toBe(true)
    })
    ...
    it('returns false for an obstacle tile', () => {
      ...
      expect(isPassable(worldEid, obstacleCol, obstacleRow)).toBe(false)
    })
  ...
  describe('bounds checks', () => {
    it('returns false for out-of-bounds coordinates', () => {
      const { worldEid } = makeWorld()
      expect(isPassable(worldEid, -1, 0)).toBe(false)
      expect(isPassable(worldEid, 0, -1)).toBe(false)
      expect(isPassable(worldEid, COLS, 0)).toBe(false)
      expect(isPassable(worldEid, 0, ROWS)).toBe(false)
    })
  })
  ...
  describe('randomPassableTile', () => {
    it('returns a tile within bounds', () => {
      const { worldEid } = makeWorld()
      const tile = randomPassableTile(worldEid)
    ...
    it('returns a passable tile', () => {
      const { worldEid } = makeWorld()
      const tile = randomPassableTile(worldEid)
      expect(isPassable(worldEid, tile.col, tile.row)).toBe(true)
    })
    ...
    it('returns different tiles across multiple calls (not always the same)', () => {
      ...
      for (let i = 0; i < 20; i++) {
        const t = randomPassableTile(worldEid)
