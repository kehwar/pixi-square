import { describe, expect, it } from 'vitest'

import { findPath } from '../pathfinder'

/** Build a grid from a 2D boolean array: true = passable, false = obstacle. */
function makeGrid(rows: boolean[][]): { isPassable: (col: number, row: number) => boolean } {
  return {
    isPassable(col: number, row: number): boolean {
      if (row < 0 || row >= rows.length)
        return false
      const r = rows[row]!
      if (col < 0 || col >= r.length)
        return false
      return r[col]!
    },
  }
}

// 5×5 grid layout (row-major, true = passable):
// . . . . .
// . X . . .
// . . . . .
// . . . X .
// . . . . .
const T = true
const F = false
const grid5x5 = makeGrid([
  [T, T, T, T, T],
  [T, F, T, T, T],
  [T, T, T, T, T],
  [T, T, T, F, T],
  [T, T, T, T, T],
])

describe('pathfinder', () => {
  it('returns a non-null path between two passable tiles', () => {
    const path = findPath(grid5x5, 0, 0, 4, 4)
    expect(path).not.toBeNull()
    expect(path!.length).toBeGreaterThan(0)
  })

  it('returns null when the destination is an obstacle', () => {
    const path = findPath(grid5x5, 0, 0, 1, 1)
    expect(path).toBeNull()
  })

  it('returns an empty array when source equals destination', () => {
    const path = findPath(grid5x5, 2, 2, 2, 2)
    expect(path).toEqual([])
  })

  it('path includes no obstacle tiles', () => {
    const path = findPath(grid5x5, 0, 0, 4, 4)
    expect(path).not.toBeNull()
    for (const tile of path!) {
      expect(grid5x5.isPassable(tile.col, tile.row)).toBe(true)
    }
  })

  it('path ends at the destination', () => {
    const path = findPath(grid5x5, 0, 0, 4, 4)
    expect(path).not.toBeNull()
    expect(path!.at(-1)).toEqual({ col: 4, row: 4 })
  })

  it('path does not include the source tile', () => {
    const path = findPath(grid5x5, 0, 0, 4, 4)
    expect(path).not.toBeNull()
    expect(path![0]).not.toEqual({ col: 0, row: 0 })
  })

  it('uses diagonal moves when they shorten the path', () => {
    // Straight diagonal from (0,0) to (2,2) — should take 2 diagonal steps
    const simpleDiag = makeGrid([
      [T, T, T],
      [T, T, T],
      [T, T, T],
    ])
    const path = findPath(simpleDiag, 0, 0, 2, 2)
    expect(path).not.toBeNull()
    // Diagonal path length should be 2 (not 4 cardinal steps)
    expect(path!.length).toBe(2)
    // Both steps should move diagonally
    expect(path![0]).toEqual({ col: 1, row: 1 })
    expect(path![1]).toEqual({ col: 2, row: 2 })
  })

  it('returns null when destination is completely surrounded by obstacles', () => {
    const isolated = makeGrid([
      [T, T, T, T, T],
      [T, F, F, F, T],
      [T, F, T, F, T],
      [T, F, F, F, T],
      [T, T, T, T, T],
    ])
    const path = findPath(isolated, 0, 0, 2, 2)
    expect(path).toBeNull()
  })

  it('routes around an obstacle in the direct path', () => {
    // Use a grid where the direct cardinal path is blocked
    const blockedMid = makeGrid([
      [T, T, T],
      [F, F, T], // blocks direct path from col=0
      [T, T, T],
    ])
    const path = findPath(blockedMid, 0, 0, 0, 2)
    expect(path).not.toBeNull()
    for (const tile of path!) {
      expect(blockedMid.isPassable(tile.col, tile.row)).toBe(true)
    }
    expect(path!.at(-1)).toEqual({ col: 0, row: 2 })
  })

  it('does not cut corners through obstacle tiles diagonally', () => {
    // Obstacle at (1,0) and (0,1) — diagonal NE from (0,1) to (1,0) must be blocked
    // Layout:
    //  . X          col 0,1 / row 0
    //  X .          col 0,1 / row 1
    // The only way from (0,0) to (1,1) that doesn't cut corners must go cardinal
    const cornerGrid = makeGrid([
      [T, F], // row 0: (0,0)=passable, (1,0)=obstacle
      [F, T], // row 1: (0,1)=obstacle, (1,1)=passable
    ])
    // No path exists: can't go diagonal (corner cut), can't go cardinal (both blocked)
    const path = findPath(cornerGrid, 0, 0, 1, 1)
    expect(path).toBeNull()
  })

  it('allows diagonal when neither cardinal neighbor is an obstacle', () => {
    // Clear 3×3 — diagonal from (0,0) to (2,2) should still work
    const open3x3 = makeGrid([
      [T, T, T],
      [T, T, T],
      [T, T, T],
    ])
    const path = findPath(open3x3, 0, 0, 2, 2)
    expect(path).not.toBeNull()
    expect(path!.length).toBe(2) // two diagonal steps
  })
})
